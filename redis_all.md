...
Back in 2009 he started the Redis project because MySQL was too slow and imperformant for his web analysis tool. [2]
...
Compared with a relational database this makes a huge difference because you never know how the query
...
Because the whole databases are stored in-memory all the time, the memory consumption is comparatively high.
...
With centralised data stores like databases new problems for multithreaded programs arrive. When concurrent access and writes takes place lost updates can occur. A simple example:
...
The problem is that you can't just compare MySQL and Redis one to one because they are different types of databases.
...
Such problems don't occur in relational databases because they are able to also select data by their value.
...
The problem of injection does not exist in Redis because the Redis protocol doesn't have a concept of string escaping. [1]
This simplifies the code and also helps developers because they don't have to think about such security issues.
